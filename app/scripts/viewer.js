...
var viewerGrid = document.getElementById('viewer-cards');
var viewerOptions = {
	grid: viewerGrid,
	viewerCounter: document.getElementById('viewer-counter'),	
	cards: viewerGrid.querySelectorAll('.viewer-item'),
	filterChips: document.getElementById('viewer-filter-chips')
};
...
function ViewerChips(opt) {
	
	var chipsListObj = {};
	var chipsArr = [];

	var parseChips = function(i, chip) {
		var chipText = chip.innerHTML;		
		var filterChips = [];
		...
		if (chipsListObj[chipText] !== null) {
			chipsListObj[chipText] = null;
		}
	};

	this.getChips = function() {
		var cL = opt.cards.length;
		var chipsStr = '';
		var filterChips = [];
		for(var i = 0; i < cL; i++) {			
			var cardChips = opt.cards[i].getAttribute('data-keywords').split(',');
		...
		var l = chipsArr.length;
 		while(l--) {
        var item = chipsArr[l];
        if(!chipsListObj[item]) {
            chipsArr.unshift(item);
            chipsListObj[item] = true;
        }
    }
    this.chips = chipsArr;
		this.chipsListObj = chipsListObj; 
	};
...
var Chips = new ViewerChips(viewerOptions);
Chips.getChips();
