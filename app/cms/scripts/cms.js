...
function processModalForm(e) {
  ...
  forEach(fieldTargets, function(index, targetField) {
    ...
    var target = $(targetId);
    if(targetId === 'tags') {
      var newTagVals = tagChipInstance.chipsData;
      console.info(newTagVals);
      insertArrayFields(newTagVals, targetId);
    } else {
...
function loadCallback(func, res) {
  ...
  for (var i = 0; i < fields.length; i++) {
    var field = fields[i];
    if(field === 'props' || (func === 'copy' && (['_id', 'fileName'].indexOf(field) > -1))) {
      continue;
    }
    var fieldObj = $(field);
    var val = resJSON[field];
    if (typeof val === 'object') {
      insertArrayFields(val, field);
    } else {
...
function insertArrayFields(val, field) {
  var existingFields = all('[data-' + field + '-input]');
  forEach(existingFields, function(index, field) {
    field.remove();
  });
  for (var j = 0; j < val.length; j++) {
    var itemVal = (val[j].tag)? val[j].tag : val[j];
    console.info(itemVal);

    var itemName = field + '[' + j + ']';
    var input = document.createElement('input');
    input.setAttribute('type', 'hidden');
    input.setAttribute('name', itemName);
    input.setAttribute('value', itemVal);
    input.setAttribute('data-' + field + '-input', '');
    editorFormElem.appendChild(input);
  }
}
    ...
    modalInstances[index] = M.Modal.init(modal, {
      onOpenStart: function(modal) {
        ...
        forEach(fieldTargets, function(index, modalField) {
          ...
          var inputType = modalField.getAttribute('data-input-type');
          if(inputType === 'tag-chips') {
            populateModalChipTags(modalField);
          } else {
            var editorField = $(targetId);
            setValue(modalField, editorField.value);
          }
          if (modalField.nextElementSibling.tagName === 'LABEL') {
...
function populateModalChipTags(modalField) {
  var tagChipsElem = one('.tag-chips');
  tagChipInstance = M.Chips.init(tagChipsElem, {
    placeholder: '',
    secondaryPlaceholder: '+tag',
  });
  var tagFields = all('[data-tags-input]');
  forEach(tagFields, function(index, tagField) {
    tagChipInstance.addChip({tag:tagField.value});
  });
}
